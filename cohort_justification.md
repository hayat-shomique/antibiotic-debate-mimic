...
## What the content hash covers, and what it does not

The frozen hash covers `inputs/cohort_skeleton.parquet`, the 7,796-episode skeleton. It does **not** cover the sampling frame or the 200-case draw, which are produced by later filters. Those are reproducible from the skeleton plus the fixed seed, and the selection code is in the repository, but the hash alone does not certify them. Stating this precisely matters: a hash that covers a superset is a weaker guarantee than it looks, and the guarantee it does give is reproducibility of the draw, not pre-registration of the frame.
